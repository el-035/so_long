...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/23 15:38:22 by efittant          #+#    #+#             */
/*   Updated: 2024/12/23 15:38:29 by efittant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_input(char *map_file)
{
	...
	if (map_file[i] != 'r' || map_file[i - 1] != 'e' || map_file[i - 2] != 'b' \
		|| map_file[i - 3] != '.')
	{
...
int	main(int argc, char **argv)
{
	...
	data = save_more_images(data);
	data = map(data, argv[1]);
	data.window = mlx_new_window(data.mlx, (data.l_len * data.tile_width), \
					(data.l_count * data.tile_height), "so_long");
	if (!data.window)
		errors("Allocation failed", data);
